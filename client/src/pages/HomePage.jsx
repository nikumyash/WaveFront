...
const HomePage = () => {
  ...
  const topics = useSelector(state=>state?.user?.user?.followedTopics) || [];
  const [sp,setSp] = useSearchParams();
  const [blogs,setBlogs] = useState([]);
  const [needFetching,setNeedFetching] = useState(false);
  const [end,setEnd] = useState(false);
  useEffect(()=>{
    customAxios.get("/blog?topic="+(sp.get("topic")||"")).then(res=>{
  ...
  useEffect(()=>{
    if(needFetching && !end){
      customAxios.get("/blog?topic="+(sp.get("topic")||"")+"&offset="+blogs.length).then(res=>{
        if(res.data.blogs.length < 8){
          setEnd(true);
        }
  ...
  return (
    <section className="flex mx-auto max-w-7xl min-h-screen justify-evenly">
        <div className="w-full flex justify-center max-w-[900px] flex-1">
          <div className="lg:mx-16 w-full">
            <TabSwitch topics={["Home",...topics.map((e)=>e.name)]}/>
            <div className="w-full my-8">
              {blogs && blogs.map((blog,i)=><BlogCard key={i} publishedAt={blog.publishedAt} blogId={blog.blogId} title={blog.title} authorUsername={blog?.author?.username} topics={blog.topics} previewImg={blog.previewImg} profileImg={blog.author.profileImg} authorName={blog.author.name} subtitle={blog.subtitle}/>)}
        ...
        <div className="w-[300px] h-[90vh] max-lg:hidden border-l border-gray-200 flex-none px-6">
          <div className="w-full h-full flex flex-col gap-4 relative my-8">
            <PopularBlog/>  
