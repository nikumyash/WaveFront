...
import { handleGoogleAuthCheck, loginUser } from "../slice/userSlice";
...
const SigninModal = () => {
  ...
  const modalData = useSelector(state=>state.modal.signinModal);
  const onSubmit =(data)=>{
  ...
  const handleGoogleAuth = ()=>{
    authWithGoogle().then((result)=>{
        ...
        dispatch(setSigninModal({isOpen:!modalData.isOpen,data:null}))
      }
    ).catch(()=>{
  ...
  return ( 
    <Dialog.Root open={modalData.isOpen} onOpenChange={()=>dispatch(setSigninModal({isOpen:!modalData.isOpen,data:null}))}>
    <Dialog.Portal>
      <Dialog.Overlay/>
      <Dialog.Content className="fixed left-[50%] bg-white top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg sm:rounded-lg">
        <h1 className='text-3xl py-4 font-serif text-center'>Welcome Back!</h1>
        <div>
          <h2 className="m-auto text-md text-center w-64 mb-4">
            ...
            <div className="mt-6 text-gray-500 text-md text-center">
                No account ?
                <span  className="text-green-700 font-medium ml-1 cursor-pointer" onClick={()=>{dispatch(setSignupModal({isOpen:true,data:null})); dispatch(setSigninModal({isOpen:false,data:null}))}}>Create one</span>
            </div>
        </div>
