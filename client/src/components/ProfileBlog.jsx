...
const ProfileBlog = () => {
  ...
  const fetchBlogs = useCallback( async ()=>{
    try{
      let res = await customAxios.get("/blog/u/"+slug);
  ...
  useEffect(()=>{
    if(needFetching && !end){
      customAxios.get("/blog/u/"+slug+"?offset="+blogs.length).then(res=>{
  ...
  return (
    <div className="w-full my-8">
      {blogs && blogs.map((blog,i)=><BlogCard key={i} publishedAt={blog.publishedAt} blogId={blog.blogId} title={blog.title} topics={blog.topics} previewImg={blog.previewImg} profileImg={blog.author.profileImg} authorName={blog.author.name} authorUsername={blog.author.username} subtitle={blog.subtitle}/>)}
