...
const UserDisplay = ({q}) => {
   ...
   useEffect(()=>{
    customAxios.get("/user?q="+q).then(res=>{
      if(res.data.users.length < 8){
        setEnd(true);
      }
   ...
   useEffect(()=>{
    if(needFetching && !end){
      customAxios.get("/user?q="+q+"&offset="+users.length).then(res=>{
        if(res.data.users.length < 8){
          setEnd(true);
        }
  ...
  return (
    <div className="w-full my-8">
        {/* {users && users.length===0 && <div className="w-full text-center text-lg">No users found</div>} */}
        {users && users.map((user,i)=><UserCard key={i} name={user.name} username={user.username} profileImg={user.profileImg} bio={user.bio}/>)}
        {users && users.length==0 && <div className="text-lg text-center">No users found</div>}
