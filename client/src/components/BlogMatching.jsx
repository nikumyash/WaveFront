...
const BlogMatching = ({q}) => {
  const timeAgo = new timeSetter('en-US')
   const [blogs,setBlogs]= useState();
   useEffect(()=>{
    customAxios.get("/blog?limit=3&q="+q).then(res=>{
  ...
  return (
    <div className="w-full">
      <h1 className="font-semibold text-lg">Posts matching {q}</h1>
        ...
        <Link to={"/search?q="+q} className="text-green-700 font-semibold">Show More</Link>
    </div>
  )
}

export default BlogMatching